jQuery(document).ready(function($) {
	 ...
	 *
	 */
	$(document).on('change', '#nwp-asw-updater tbody input[type="checkbox"]', function() {
		if ($('#nwp-asw-updater tbody input[type="checkbox"]:checked:not(:disabled)').length)
			$('#nwp-do-asw-update').prop('disabled', false);
		else
			$('#nwp-do-asw-update').prop('disabled', true);
	});
	 ...
	 */
	$(document).on('change', '#nwp-asw-updater tbody input[type="checkbox"]', function() {
		let checked_cbs = $('#nwp-asw-updater tbody input[type="checkbox"]:checked:not(:disabled)');
		$('#nwp-do-asw-update').prop('disabled', checked_cbs.length == 0 ? true : false);
	});

	// Open up panel and show all sizes for a particular color
	$(document).on('click', '#nwp-asw-updater .toggle-indicator', function() {
		$(this).toggleClass('open').parents('tbody').find('.nw-update-image-row').toggle();
	});
	...
	$(document).on('change', '#nwp-asw-updater #nw_update_images', function() {
		$('#nwp-asw-updater .nw-update-image-row input:not(:disabled)').prop('checked', $(this).prop('checked')).trigger('change');
	});
});
