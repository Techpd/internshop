...
<div class="images">
    ...
    <div class="row">
        ...
        if ($product->is_type('stock_item') || $product->is_type('stock_item_logo')){
        ...
        if (( $attachment_count + $i_count) > 1) {
            ...
            <div class="col-md-10 col-sm-10 col-md-push-2 col-sm-push-2 custom-width-thumb">
                <div class="slider-for single_thumb_wrap">
                    ...
                    if (!$is_color_selected && has_post_thumbnail() && !$product->is_type('stock_item') && !$product->is_type('stock_item_logo')) {
                        ...
                        $classes[] = 'first';
                        echo '<div class="item">';
                        echo '<div class="item-inner img-first">';
                    ...
                    if ($product->is_type('stock_item') || $product->is_type('stock_item_logo')) {
                        ...
                        echo '<div class="item">';
                        echo '<div class="item-inner img-first-new">';
                        echo '<a href="' . $image_link . '" class="mfp-image" data-rel="prettyPhoto">';
                        echo '<img class="img-responsive" src="' . $image_link . '">';
                        echo '</a>';
                        echo '</div>';
                        ...
                        foreach ($attachment_ids as $attachment_id){
                            ...
                            echo '<div class="item"><a href="' . $img . '" class="mfp-image img-mfp-new" data-rel="prettyPhoto"><img class="img-responsive " src="' . $img . '"></a></div>';
                        }
            ...
            <div class="col-sm-12">
                <div class="single_thumb_wrap no-slider-nav">
                    <?php
//                    $dummy_image = get_template_directory_uri() . '/images/dummy.png';
                    if (has_post_thumbnail() && !$product->is_type('stock_item') && !$product->is_type('stock_item_logo')) {
                        $image_title = esc_attr(get_the_title(get_post_thumbnail_id()));
                        $image_caption = get_post(get_post_thumbnail_id())->post_excerpt;
                        $image_link = wp_get_attachment_url(get_post_thumbnail_id());
                    ...
                    elseif($product->is_type('stock_item') || $product->is_type('stock_item_logo')){
                        $image_link = isset($selected_variation['image']['full_src']) ? $selected_variation['image']['full_src'] : $post_thumbnail_url;
...
//                $attachment_count = count($attachment_ids);
                ...
                if (( $attachment_count + $i_count) > 1) {
                    ...
                    if (!$is_color_selected && !$product->is_type('stock_item') && !$product->is_type('stock_item_logo')) {
                        foreach ($attachment_ids as $attachment_id) {
                            ?>
                            <div class="item">
                                <?php
