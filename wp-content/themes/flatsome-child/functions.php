...
function new_home_styles()
{
    ...
    wp_enqueue_script('script_js', get_stylesheet_directory_uri() . '/js/script-js.js', false, time());
    ...
    wp_localize_script('script_js', 'myAjax', array('ajaxurl' => admin_url('admin-ajax.php')));

    if (is_single()) {
        // wp_dequeue_style( 'flatsome-main' );
        wp_enqueue_style('flatsome-custom', get_stylesheet_directory_uri() . '/css/flatsome.css', false, time());
    }
}
add_action('wp_enqueue_scripts', 'new_home_styles');

// Function to dequeue parent theme's CSS file
function dequeue_parent_theme_css()
{
    if (is_single()) {
        wp_dequeue_style('flatsome-main');
    }
    // Dequeue 'flatsome-main' CSS file
}
// Hook the function to run after the parent theme's styles are enqueued
add_action('wp_enqueue_scripts', 'dequeue_parent_theme_css', 999);
...
?>
    <div class="left-check-col">
        ...
        <div class="after-checkout-gift-card-form">
            ...
            <div class="ywgc_enter_code" style="display: none;"><!--style="display: none;"-->
                <input type="text" name="gift_card_code" class="input-text" placeholder="<?php echo esc_attr(apply_filters('ywgc_checkout_box_placeholder', _x('Gavekortkode', 'Apply gift card', 'yith-woocommerce-gift-cards'))); ?>" id="giftcard_code" value="" />
    ...
    float: left;
                                                                                                        ...
                                                                                                        add_action('wp_head', 'add_cookie_consent_script');
    ...
    <?php
                                                                                                        ...
                                                                                                        //Extend general tab to add cb for new product gallery
                                                                                                        add_action('woocommerce_product_options_general_product_data', 'extend_general_panel', 99);

                                                                                                        function extend_general_panel()
                                                                                                        {
                                                                                                            global $product_object;
                                                                                                            $checked = (get_post_meta($product_object->get_id(), "_show_slick_slider_gallery", true) == 1) ? "checked" : "";
    ?>
        <div class="options_group">
            <p class="form-field slick_slider_gallery_cb">
                <label for="show_slick_slider_gallery"><?php _e('Use new method for displaying product gallery', 'newwave'); ?></label>
                <input type="checkbox" id="show_slick_slider_gallery" name="show_slick_slider_gallery" value="1" <?php echo $checked; ?> />
            </p>
        </div>
    <?php
                                                                                                        }

                                                                                                        add_action('woocommerce_admin_process_product_object', 'save_slider_cb_val', 100);

                                                                                                        function save_slider_cb_val($product)
                                                                                                        {
                                                                                                            $show_slick_slider_gallery = "0";
                                                                                                            if (isset($_POST['show_slick_slider_gallery'])) {
                                                                                                                $show_slick_slider_gallery = "1";
                                                                                                            }
                                                                                                            if (get_post_meta($product->get_id(), '_show_slick_slider_gallery', true) == "") {
                                                                                                                add_post_meta($product->get_id(), '_show_slick_slider_gallery', $show_slick_slider_gallery);
                                                                                                            } else {
                                                                                                                update_post_meta($product->get_id(), '_show_slick_slider_gallery', $show_slick_slider_gallery);
                                                                                                            }
                                                                                                        }

                                                                                                        /*
 * Custom Color Variants Gallery
 */

                                                                                                        add_filter('acf/load_field/name=product_color', 'acf_load_color_field_choices');
                                                                                                        function acf_load_color_field_choices($field)
                                                                                                        {
                                                                                                            $field['choices'] = array();
                                                                                                            $pid = get_the_ID();
                                                                                                            $choices = wc_get_product_terms($pid, 'pa_color');
                                                                                                        ...
                                                                                                        add_action('wp_ajax_get_meta_key_by_color', 'get_meta_key_by_color');
                                                                                                        add_action('wp_ajax_nopriv_get_meta_key_by_color', 'get_meta_key_by_color');

                                                                                                        function get_meta_key_by_color()
                                                                                                        {
                                                                                                            $pa_color = sanitize_text_field($_POST['pa_color']);
                                                                                                            $product_id = intval($_POST['product_id']);

                                                                                                            $show_slick_slider_gallery = get_post_meta($product_id, '_show_slick_slider_gallery', true);

                                                                                                            if ($show_slick_slider_gallery) {
                                                                                                                // Get the color ID based on the slug
                                                                                                                $term = get_term_by('slug', $pa_color, 'pa_color');
                                                                                                                if (!$term) {
                                                                                                                    wp_send_json_error('Color not found');
                                                                                                                }
                                                                                                                $color_id = $term->term_id;

                                                                                                                // Find the meta key where meta_value matches the color ID
                                                                                                                global $wpdb;
                                                                                                                ...
                                                                                                                if ($meta_key) {
                                                                                                                    ...
                                                                                                                    if ($matches && isset($matches[1])) {
                                                                                                                        ...
                                                                                                                        $attachment_ids =  get_post_meta($product_id, 'color_variants_gallery_' . $number . '_color_gallery', true);

                                                                                                                        if ($attachment_ids) {
                                                                                                                            // Get the image URLs for each attachment ID
                                                                                                                    ...
                                                                                                                    wp_send_json_error('Meta key not found');
                                                                                                                }
                                                                                                            } else {
                                                                                                                wp_send_json_success('slick not workin');
                                                                                                            }
                                                                                                        }


                                                                                                        add_action("wp_ajax_change_variant_gallery", "change_variant_gallery");
                                                                                                        add_action("wp_ajax_nopriv_change_variant_gallery", "change_variant_gallery");
                                                                                                        function change_variant_gallery()
                                                                                                        {
                                                                                                            ob_start();
